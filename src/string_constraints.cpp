...
static uint8_t regnum(const string& s) { return (uint8_t)boost::lexical_cast<uint16_t>(s.substr(1)); }

static crab::data_kind_t regkind(const string& s) {
    if (s == "type")
        return crab::data_kind_t::types;
    if (s == "offset")
        return crab::data_kind_t::offsets;
    if (s == "region_size")
        return crab::data_kind_t::region_size;
    if (s == "value")
        return crab::data_kind_t::values;
...
static type_encoding_t string_to_type_encoding(const string& s) {
    static map<string, type_encoding_t> string_to_type{
        {string("uninit"), T_UNINIT}, {string("map_fd_programs"), T_MAP_PROGRAMS},
        {string("map"), T_MAP},       {string("number"), T_NUM},
        {string("ctx"), T_CTX},       {string("stack"), T_STACK},
        {string("packet"), T_PACKET}, {string("shared"), T_SHARED},
    };
...
std::vector<linear_constraint_t> parse_linear_constraints(const std::set<string>& constraints,
    ...
    for (const string& cst_text : constraints) {
        std::smatch m;
        if (regex_match(cst_text, m, regex("false"))) {
            res.push_back(linear_constraint_t::FALSE());
